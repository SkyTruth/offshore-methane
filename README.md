...
# Optional: create a `.env` file containing `CDSE_USERNAME` and
# `CDSE_PASSWORD` if you plan to download from the Copernicus
...
pytest
```
The test suite expects the optional `geojson` package. Install it with
`pip install geojson` if you see import errors.
