...
def sentinel2_geemap(id):
    ...
    if id.startswith(("S2A", "S2B")):
        ...
        imDate_form = datetime.datetime(
            int(imDate[0:4]), int(imDate[4:6]), int(imDate[6:])
        )
...
def calculateSunglint_alpha(image):
    ...
    theta_naught = (
        ee.Number(image.get("MEAN_SOLAR_ZENITH_ANGLE")).multiply(math.pi).divide(180)
    )
    phi_naught = (
        ee.Number(image.get("MEAN_SOLAR_AZIMUTH_ANGLE")).multiply(math.pi).divide(180)
    )
...
def get_grid_values_from_xml(tree_node, xpath_str):
    ...
    for node in node_list:
        ...
        values_arr = np.array(list(map(lambda x: x.split(" "), values_lst))).astype(
            "float"
        )
...
def create_sunglint_map_s2(index):
    ...
    view_zenith = get_grid_values_from_xml(
        root, ".//Viewing_Incidence_Angles_Grids/Zenith"
    )
    view_azimuth = get_grid_values_from_xml(
        root, ".//Viewing_Incidence_Angles_Grids/Azimuth"
    )
    ...
    with rasterio.open(
        ...
        cog_translate(
            dst, source_file_name, cog_profiles.get("deflate"), in_memory=False
        )
